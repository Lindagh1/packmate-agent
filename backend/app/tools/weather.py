...
async def get_weather(city: str, days: int = 14) -> WeatherResponse:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            return WeatherResponse(location=loc["name"], forecast=forecasts)
    except CityNotFoundError:
        raise
    except Exception as exc:
        raise WeatherToolError(f"Tool error: {exc}") from exc
