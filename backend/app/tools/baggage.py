...
def _matches_baggage_type(rule_types: list[str], baggage_type: str) -> bool:
    return baggage_type in rule_types
...
def _liquid_over_100ml_warnings(item: str, baggage_type: str) -> list[str]:
...
def lookup_baggage_rules(
    ...
    for rule in rules_data["rules"]:
        if not _matches_baggage_type(rule["baggage_types"], baggage_type):
            continue
