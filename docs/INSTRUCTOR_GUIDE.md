...
`scripts/promote-backend-image.sh --create-pr` and `scripts/rollback-prod-image.sh --create-pr` push a branch to `origin` and call `gh pr create` against `packmate-v2`. This requires:

- `gh auth login` completed with a token that has **write access** to the target repository (push branch + open PR), on the machine/Workbench running the script; or
...
The reference model for this lab is **one ephemeral sandbox cluster per participant** (for example an OpenTLC-style sandbox). On that model there is no naming collision: each participant's cluster has exactly one `packmate-lab`, one `packmate-prod`, one AppProject `packmate`, and one Application `packmate-prod`.
...
- Recommended patterns, in order of preference:
  1. **One sandbox per participant** (no changes needed) — the supported/validated path.
  2. **One shared cluster, single shared PROD** — only the instructor (or one nominated participant) actually merges and Syncs to `packmate-prod`; everyone else follows along and reviews PRs. Simple, matches the "one production" story, no manifest changes.
...
- For the **GitHub repo** side of a multi-user class: either give every participant collaborator (write) access to a shared class repository/branch so `--create-pr` works directly against `packmate-v2`, or have each participant fork the repo and open the PR from their fork (`gh pr create --repo Lindagh1/packmate-agent --head <fork-owner>:promote/backend-<digest>`), with a required review before merge. Branch protection on `packmate-v2` (require PR review, no direct pushes) is recommended either way so Module D's review step is enforced, not just suggested.

## Same-repo vs separate GitOps repo

Packmate currently ships **manifests and application code in the same repository** (`deploy/`, `argocd/` next to `backend/`, `frontend/`, `mcp-servers/`). This is what `scripts/promote-backend-image.sh` and `scripts/rollback-prod-image.sh` assume (`BASE_BRANCH="packmate-v2"`, edits under `deploy/overlays/prod/`).
